...
func main() {
	...
	mst, edges := Prims(img)
	Genomes := Cutter(mst, edges, popSize, 0, 1)
	pop := make([]Solution, popSize, popSize)
	pop[0].Genome = Genomes[0]
	segments := findSegments((pop[0]), &img)
	fmt.Println(" ")
	fmt.Print(len(segments))
	fmt.Println(" ")
	fmt.Print(img.Bounds().Max.X * img.Bounds().Max.Y)
	fmt.Println(" ")
